...
# fazer publicacao
function setterArtigo($user, $connect)
{
    # busca todos os dados do formulario
    $dados = filter_input_array(INPUT_POST, FILTER_UNSAFE_RAW);
    //var_dump($dados);
    ...
    query("INSERT INTO `artigos` (`usuario`, `categoria`, `titulo`, `descricao`) VALUES('{$user['id']}', '{$dados['categoria']}', '{$dados['titulo']}', '{$dados['descricao']}');", $connect, null, $erros, false);
}

# fazer publicacao
function setterPlaylist($user, $connect)
{
    # busca todos os dados do formulario
    $dados = filter_input_array(INPUT_POST, FILTER_UNSAFE_RAW);
    //var_dump($dados);
    ...
    query("INSERT INTO `playlists` (`usuario`, `url`, `titulo`, `descricao`) VALUES('{$user['id']}', '{$dados['url']}', '{$dados['titulo']}', '{$dados['descricao']}');", $connect, null, $erros, false);
}

# atualizar dados do advogado
# funcao para cadastrar turma
function updateAdvogado($user, $connect)
{
    # busca todos os dados do formulario
    $dados = filter_input_array(INPUT_POST, FILTER_UNSAFE_RAW);
    //var_dump($dados);
    ...
    atualizar('usuarios', [
        ...
        'profissao' => $dados['profissao'] ?? ($user['profissao'] ?? NULL),
        'idiomas' => $dados['idiomas'] ?? ($user['idiomas'] ?? NULL),
        'experiencia' => $dados['experiencia'] ?? ($user['experiencia'] ?? NULL),
        'atividade_extra' => $dados['atividade_extra'] ?? ($user['atividade_extra'] ?? NULL),
        'modalidade' => $dados['modalidade'] ?? ($user['modalidade'] ?? NULL),
        'valor_consulta' => $dados['valor_consulta'] ?? ($user['valor_consulta'] ?? NULL),
        'formacao' => $dados['formacao'] ?? ($user['formacao'] ?? NULL),
        'desconto' => $dados['desconto'] ?? ($user['desconto'] ?? NULL)
    ], $user['id'], $erros, null, false, $connect);
